...
export class RangeProofGenerator {
   ...
   */
  static generate(value: bigint, deviceId: string, lowerBound: bigint, upperBound: bigint): Buffer {
    // 16-byte commitment
    const cmtInput = Buffer.from(`cmt:${value}`);
    ...
    const chInput = Buffer.concat([
      commitment,
      Buffer.from(`:${deviceId}:${lowerBound}:${upperBound}`)
    ]);
    const challenge = Buffer.from(nacl.hash(chInput)).subarray(0, 16);

    // 32-byte response
    const respInput = Buffer.concat([challenge, Buffer.from(`:${value}`)]);
...
export class ZkRangeProofVerifier {
   ...
   */
  verifyRangeProof(proofBuffer: Buffer, deviceId: string, lowerBound: bigint, upperBound: bigint): VerificationResult {
